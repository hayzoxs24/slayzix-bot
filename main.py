        ...
        for target, overwrite in list(channel.overwrites.items()):
            if isinstance(target, discord.Member):
                ...
                if target.guild_permissions.administrator:
                    continue
                if ticket_owner and target == ticket_owner:
                    continue
        ...
        async def select_callback(self, interaction):
            ...
            try:
                if field == "title":         self.embed.title = val
                    ...
                    self.embed.remove_field(int(val) - 1)
            except Exception:
...
async def help_cmd(interaction):
    ...
    e.add_field(name="📢 Divers",value="`*say` `*announce` `*dmall` `*tos` `*wearelegit` `*stats` `*userinfo` `*serverinfo` `*prefix` `*shopembed`",inline=False)
